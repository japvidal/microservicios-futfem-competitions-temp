...
public class CompetitionController extends CommonController<Competition, CompetitionService> {
	...
	@PostMapping("/search")
	public ResponseEntity<?> search(@RequestBody(required = false) CompetitionSearchRequest request) {
		return ResponseEntity.ok(service.searchCompetitions(request == null ? new CompetitionSearchRequest() : request));
	}
